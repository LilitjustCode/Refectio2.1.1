...
export default class EditPhoneNumberConfirmComponent extends React.Component {
  ...
  sendPhoneCode = async () => {
    ...
    await fetch(`https://admin.refectio.ru/public/api/updatePhoneNumberProizvoditel`, requestOptions)
  ...
  updateCodeSend = async () => {
    if (this.state.timerBool == true) {
      ...
      await fetch(`https://admin.refectio.ru/public/api/updateCodeIntestTable`, requestOptions)
        .then(response => response.json())
        .then(result => {
